...
public void setup() {
  ...
  //mayaModel = new OBJModel(this,"./data/terrain.obj", "relative", QUADS);
  mayaCube = new OBJModel(this, "./data/cube.obj", "TRIANGLES");
  bbox = new BoundingBox(this, mayaCube);
  objList = new ArrayList<OBJModel>();
  
  //mayaModel.enableDebug();
  //mayaCube.enableDebug();
...
// TEST CODE FOR JOINT ROTATION // 
  ...
  JointUtils.calcJointRotation(testJoint1, testJoint2);
  float[] m = new float[3];
  m = JointUtils.rotMatrix;
  gl.glMultMatrixf(m,0);
  println("current rot matrix: ");
  println(m);
}
...
//public boolean hasCollided(PVector joint, OBJModel obj)
//{
//  for(int i = 0; i < jointList.size(); i++)
//  {
//    joint = jointList.get(i);
...
void drawPoints(int userId)
{
  ...
  pushMatrix();
    translate(jointPos_leftHand.x/20, jointPos_leftHand.y/20, jointPos_leftHand.z/500);
    sphere(.5);
  popMatrix();
  
  pushMatrix();
    translate(jointPos_rightHand.x/20, jointPos_rightHand.y/20, jointPos_rightHand.z/500);
    sphere(.5);
  popMatrix();
